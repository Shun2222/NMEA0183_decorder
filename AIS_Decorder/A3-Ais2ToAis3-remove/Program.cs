    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (StreamWriter swErr = new StreamWriter(errFile, true))
            {
                ...
                {
                    logout("Creat black list mode = false");
                }
                ...
                using (StreamWriter sw = new StreamWriter(outFile))
                {
                    ...
                    count = 0;
                    dataCount = 0;
                    int newMmsiGridCount = 0;
                    logout(string.Format("BrokenMmsiNum: {0}", blm2.blackList.Count));
                    // debug用ログの出力
                    if (DEBUG)
                    {
                        string ss = "AllBrokenMMSI:";
                        foreach (UInt32 bmmsi in blm2.blackList)
                        {
                            ss += bmmsi.ToString();
                            ss += ", ";
                        }
                    ...
                    foreach (DefineMeshArea.GridIdx tidx in tidxs)
                    {
                        ...
                        // 履歴にないMMSIが存在する場合は悪いMMSIかを判断
                        var mmsiSumValue = g.getMmsiSumValue();
                        if (!CREATE_BLACKLIST)
                        {
                            bool existMmsi = true;
                            foreach (UInt32 mmsi in g.getMmsi())
                            {
                                if (!blm2.isContainInMmsi(mmsi))
                                {
                                    existMmsi = false;
                                    newMmsiGridCount += 1;
                                }
                        ...
                        // ブラックリストのMMSIを排除して偏流値の計算
                        var mmsiVecDict = g.calcCurMmsiLambda(mmsiSumValue);
                        var whiteBlackMmsi = g.mmsiSelection(blm2.blackList);
                        XY cur = g.calcCurLSMTargetMmsi(mmsiVecDict, whiteBlackMmsi[0]);

                        string addBlackMmsiString = "";
                        bool isFixedCur = false;

                        // 偏流値が異常な場合に，排除したMMSIが良かった場合を考慮する
                        var whiteMmsi = whiteBlackMmsi[0];  
                        if (!isValidCurValue(cur) && whiteBlackMmsi[1].Count > 0) 
                        {
                            foreach (UInt32 mmsi in whiteBlackMmsi[1]) 
                            { 
                                whiteMmsi = whiteBlackMmsi[0];  
                                whiteMmsi.Add(mmsi);
                                var cur2 = g.calcCurLSMTargetMmsi(mmsiVecDict, whiteBlackMmsi[0]);
                                if (isValidCurValue(cur2))
                                {
                                    cur = cur2;
                                    addBlackMmsiString = "AddedBlackMMSI:";
                                    addBlackMmsiString += mmsi.ToString();
                                    isFixedCur = true;
                                    break;
                                }
                            }
                        ...
                        if (DEBUG)
                        {
                            ...
                            if (!isValidCurValue(cur) && whiteBlackMmsi[1].Count>0)
                            {
                                ...
                                {
                                    foreach (UInt32 bMmsi in whiteBlackMmsi[1]) 
                                    { 
                                        ss += bMmsi.ToString();
                                        ss += ", ";
                                    }
