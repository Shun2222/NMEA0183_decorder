    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            using (StreamWriter swErr = new StreamWriter(errFile, true))
            {
                ...
                //ファイルの読み込み
                List<UInt32> dummyOriginalMmsiList = new List<UInt32> {431400179, 431401051, 477749200};
                ...
                foreach (string inFile in inFiles)
                    using (StreamReader sr = new StreamReader(inFile))
                    {
                        ...
                        do
                        {
                            ...
                            if (dummyOriginalMmsiList.Contains(saMmsi) && aisr.Valid)
                            {
                                int index = dummyOriginalMmsiList.IndexOf(saMmsi);
                                string[] sa2 = new string[sa.Length] ;
                                Array.Copy(sa , sa2, sa.Length);
                                sa2[2] = dummyMmsiList[index].ToString();
                        ...
                        do
                        {
                            ...
                            if (dummyOriginalMmsiList.Contains(mmsi)) { 
                                Console.WriteLine("\nMMSI:{0}, Count:{1}, DT:{2}-{3}\n", mmsi, validCount, AISList[0].LatLonDT.DT, AISList[AISList.Count-1].LatLonDT.DT);
                            }
                ...
                foreach (KeyValuePair<UInt32, int> dicItem in mmsiCount)
                { 
                    if (dicItem.Value > 3000)
                    Console.WriteLine("MMSI:{0}, Count:{1}\n", dicItem.Key, dicItem.Value);
                }
